...
export default function TripCard({ trip, variant = "default", className = "" }) {
  ...
  return (
    <div
      ...
      className={`ticket-card group relative hover-lift fade-in-up ${className}`.trim()}
    >
      <div className="ticket-content">
        <div className="relative h-44 overflow-hidden rounded-t-[1.5rem]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#12213F]/80 via-[#12213F]/20 to-transparent" />
          <div className="absolute top-4 left-4">
            <span className={`px-3 py-1 rounded-full text-[10px] font-semibold tracking-[0.18em] uppercase ${
              trip.status === "upcoming" ? "bg-[#276667] text-white" :
              trip.status === "over-budget" ? "bg-[#C43E2E] text-white" :
              "bg-[#F1ECDF]/85 text-[#12213F]"
            }`}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className="w-9 h-9 rounded-full grid place-items-center bg-[#F1ECDF]/90 hover:bg-white transition-colors"
                  aria-label="Trip options"
                >
                  <MoreHorizontal className="w-4 h-4 text-[#12213F]" />
                </button>
              </DropdownMenuTrigger>
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white">
            ...
            <div className="flex items-center gap-3 text-[11px] mt-2 opacity-90 font-mono uppercase tracking-[0.18em]">
        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center gap-4 text-[11px] text-[#20211D]/70 font-mono uppercase tracking-[0.15em]">
          ...
          <div>
            <div className="flex items-baseline justify-between mb-1.5">
              <span className="text-[10px] uppercase tracking-[0.24em] text-[#20211D]/70 font-semibold font-mono">Budget</span>
              <span className={`text-sm font-semibold ${over ? "text-[#C43E2E]" : "text-[#12213F]"}`}>
                ₹{trip.estimatedCost.toLocaleString()} / ₹{trip.budget.toLocaleString()}
              </span>
            </div>
            <div className="h-2 rounded-full bg-[#E7DFC9] overflow-hidden">
              <div className={`h-full rounded-full transition-all ${over ? "bg-[#C43E2E]" : "bg-[#B8862F]"}`} style={{ width: `${pct}%` }} />
            </div>
          </div>
          ...
          {variant !== "compact" && (
            <div className="flex items-center gap-2 pt-1">
              <Link
                ...
                className="flex-1 text-center py-2.5 rounded-full bg-[#12213F] text-[#F1ECDF] text-sm font-semibold hover:opacity-95 transition-colors"
              >
                View Trip
              </Link>
              <Link
                ...
                className="flex-1 text-center py-2.5 rounded-full border border-[#d8cfbc] text-sm font-semibold hover:bg-[#E4DDCD] transition-colors"
              >
                Edit
              </Link>
            </div>
